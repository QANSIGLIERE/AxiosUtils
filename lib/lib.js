...
async function get(baseURL, additionalPath, headers, protocol = 'https', returnAll = false, ignoreIssues = false) {
    let options = {
    ...
    if (returnAll) {
        return { status: response.status, response: serverResponse, headers: response.headers };
    } else {
