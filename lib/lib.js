...
async function download(baseURL, additionalPath, headers, keyToCollect, keyToNext) {
    ...
    console.log(Object.keys(initialCall.response));
...
(async function Demo() {
    let value = await get(
        'https://10-lte.revelup.com',
        'ap/ok',
        {
            'content-type': 'application/json',
            'API-AUTHENTICATION':
                'f0919d9c19924e069bd5b195ad953436:5db42581580a482e9d7018d7ffce662c541f79f0ac104ed8ac874e0efc255de0',
        },
        true,
    );
    console.log(value);
...
['hostname', 'syscall', 'code', 'errno', 'message', 'name', 'config', 'request', 'cause']; // not found
['message', 'name', 'code', 'config', 'request', 'response', 'status']; // 404
['status', 'statusText', 'headers', 'config', 'request', 'data']; // 200
