...
var { linkParserFromResponse } = require('qansigliere-launchdarkly-parse-link-from-response');
...
function getNextLink(headersObject, responseObject, mode) {
    ...
    switch (mode) {
        ...
        case 'launchdarkly':
            if (responseObject) {
                finalResult = Object.keys(linkParserFromResponse(responseObject)).includes('next')
                    ? linkParserFromResponse(responseObject).next.replace('/api/v2/', '')
                    : null;
            }
            break;
    }
